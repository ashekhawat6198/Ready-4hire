...
const evaluateAnswerAsync = async (
  ...
  try {
    ...
    if (session.status === "completed" || allQuestionsEvaluated) {
      ...
      if (!session.isScoreCounted) {
        const user = await user.findById(userId);

        if (user) {
          const newScore = session.overallScore;
          user.overallScore +=newScore 

          if (newScore >= 80) user.points += 20;
          else if (newScore >= 50) user.points += 10;
          else user.points += 5;

        
          await user.save();
        }
        session.isScoreCounted=true;
      }
...
const createResumeSession = asyncHandler(async (req, res) => {
  const userId = req.user._id;
  ...
  res.status(202).json({
    message: "Session Created through Resume.AI is analyzing",
    sessionId: session._id,
    status: "processing",
  });
  ...
  (async () => {
    try {
      ...
      const result = new PDFParse(uint8Array);
      ...
      const aiResponse = await fetch(`${AI_SERVICE_URL}/resume-interview`, {
        method: "POST",
        headers: {
